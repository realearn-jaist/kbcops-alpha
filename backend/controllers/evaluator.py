...
class InclusionEvaluator(Evaluator):
    def __init__(self, valid_samples, test_samples, train_X, train_y, classes, classes_e, inferred_ancestors, ontology, algorithm):
    ...
    def evaluate(self, model, eva_samples):
        ...
        for k, sample in enumerate(progress_bar):
            ...
            if rank_non > rank:
                ...
                DLcount += 1
                data.append({
                    'Individual': sub.split(('/'))[-1], 
                    'Predicted': predicted_inf, 
                    'Predicted_rank': predicted_inf_rank, 
                    'True': sorted_classes_non[rank_non-1].split(('/'))[-1], 
                    'True_rank' : rank_non, 
...
def predict_func(ontology, algorithm):
    ...
    classes = [line.strip() for line in load_classes(ontology)]
    ...
    evaluate = InclusionEvaluator(valid_samples, test_samples, train_X, train_y, classes, classes_e, inferred_ancestors, ontology, algorithm)
